...
name: Sysadmin's Self-Cultivation (brass)
slug: sysadmin-brass
source-url: none
created: 2026-08-25
updated: 2026-08-25
notes: Warm workbench skin: bone/iron/brass; system font fallbacks for SVG export
-->
...
This skin is a warm editorial palette — bone paper, iron ink, brass accent, ash muted. It is the skin of The Sysadmin's Self-Cultivation: metal and workbench, matching the repo's 🔨 hands-on / 🧭 verified-ramp markers. It's designed to look good out of the box; swap these values (or run [`onboarding.md`](onboarding.md)) and every new diagram inherits the new skin without touching any type-specific logic.
...
| `paper-2` | Diagram container bg, secondary fill | `#f0ece4` | `#25241f` |
| `ink` | Primary text, primary stroke | `#26262b` (iron) | `#faf8f4` (bone) |
| `muted` | Secondary text, default arrow stroke | `#5d5a52` (ash) | `#b8b2a6` |
| `soft` | Sublabels, boundary labels | `#8a8479` | `#8f887c` |
| `rule` | Hairline borders | `rgba(38,38,43,0.12)` | `rgba(250,248,244,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#c9c2b4` | `rgba(201,194,180,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#a8763e` (brass) | `#c99a5b` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(168,118,62,0.08)` | `rgba(201,154,91,0.10)` |
| `link` | HTTP/API calls, external arrows | `#4a6b8a` (patina-blue) | `#7fa3c4` |

> **Brand palette source:** a five-color workbench palette — `iron #26262b`, `ash #5d5a52`, `bone #faf8f4`, `brass #a8763e`, `patina-blue #4a6b8a`. The `soft`, `rule`, and `rule-solid` tokens are derived (lighter ash, ink-at-opacity, warm silver) to cover roles the palette does not name directly.
...
Any `rgba(38,38,43, X)` in light becomes `rgba(250,248,244, X)` in dark. Same opacities, RGB flipped. The accent gets a slight hue-shift brighter to read on dark paper.
...
|---|---|---|---|---|
| `title` | 'Instrument Serif', Georgia, 'Times New Roman', serif | 1.75rem | 400 | Page H1 |
| `node-name` | 'Geist', system-ui, -apple-system, 'Helvetica Neue', sans-serif | 12px | 600 | Human-readable labels |
| `sublabel` | 'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace | 9px | 400 | Port, protocol, URL, field type |
| `eyebrow` | 'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace | 7–8px | 500, tracked 0.18em, uppercase | Type tags, axis labels |
| `arrow-label` | 'Geist Mono', ui-monospace, SFMono-Regular, Menlo, monospace | 8px | 400, tracked 0.06em | Arrow annotations |
| `callout` | 'Instrument Serif' *italic*, Georgia italic, serif | 14px | 400 | Editorial asides only |

### Font stack

```html
<!-- Optional. Every family above ends in a system fallback, because these
     diagrams are exported to SVG and embedded in Markdown, where no webfont loads. -->
