...
| Walkthrough | [`walkthrough/`](walkthrough/README.md) — a second route, spoken rather than read: the format, the five decisions ([ADR-0009](docs/adr/0009-the-walkthrough-ships-its-script-not-its-audio.md)–[0013](docs/adr/0013-godot-is-a-design-tool-and-the-floor-keeps-one-palette.md)) and the viewer registration are written, and **two walkthroughs are built** — 01 · the network (106 beats) and 02 · the first Monday (93 beats), each in two languages over the same interactive 2D floor. Scripts ship, audio never does. Grows one walkthrough at a time like `toolbox/`, with no target count | ✅ |
