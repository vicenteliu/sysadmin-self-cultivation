...
| Site design | [`cross-cutting/site-network-design.md`](cross-cutting/site-network-design.md) — the step between the reference office's parameters and a design, at the same altitude as the layer chapter. Paired with [`the-reference-office.md`](the-reference-office.md), which now carries **six parameter domains** past the spatial ones — people flow, endpoints and spares, identity shape, the SaaS estate, support load, and data and recovery — each written because a lab or a step had already been forced to invent it, and each recorded in that file's own demand ledger. `Reference build` stays ⏳ because no step asks for a model, and `Where things run` is ⏳ beside it because nothing has yet been forced to invent this office's cloud numbers | ✅ |
| Walkthrough | [`walkthrough/`](walkthrough/README.md) — a second route, spoken rather than read: the format, the five decisions ([ADR-0009](docs/adr/0009-the-walkthrough-ships-its-script-not-its-audio.md)–[0013](docs/adr/0013-godot-is-a-design-tool-and-the-floor-keeps-one-palette.md)) and the viewer registration are written, and **walkthrough 01 is built** — 106 beats in two languages over an interactive 2D floor. Scripts ship, audio never does. Grows one walkthrough at a time like `toolbox/`, with no target count | ✅ |
