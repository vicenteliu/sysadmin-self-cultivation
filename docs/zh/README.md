...
| **带上能直接跑的工具** | [`toolbox/`](toolbox/) —— 六个发现/审计脚本、三个 Ansible 修复 roles、加一个按环境打包子集的[生成器](toolbox/generate/) |
| **把方法当工具用** | [`.claude/skills/`](../../.claude/skills/) —— 八个 Agent Skill：五个包装方法（ramp · audit · author · lab · mirror），三个驱动工具箱 |

## 已建成

roadmap 计划的都写完了，含**十二个可跑、自验证的 lab**（退出码 `0` = 教训成立）、**八篇 break-fix support 笔记**、**八个 Agent Skill**、和一个**能直接跑的工具箱**（六个脚本、三个 Ansible 修复 roles、一个按环境打包的[生成器](toolbox/generate/)）；剩下的是更多可跑 lab、完整中文镜像（本页已起步），以及按需求深化。
...
- **工具箱（拿去就能跑）** —— [charter + 约定](toolbox/README.md) · 六个脚本（分诊 · 用户 · 补丁 · 基线 · 备份演练 · cidr）· [Ansible 修复 roles](toolbox/ansible/) 与审计脚本配对（audit→fix）· 一个[按环境打包的生成器](toolbox/generate/) —— 安全默认，每个工具带自己的 `Tested on:` 行 ✅
