...
| **把方法当工具用** | [`.claude/skills/`](../../.claude/skills/) —— 四个可调用的 Agent Skill（ramp · audit · author · lab） |
...
| --- | --- | --- | --- | --- |
| **[AWS](../../platforms/aws/)**（完整样板） | ✅ | ✅ ✅ ✅ | ✅ 3-lab arc —— **2 个可跑**（boto3 + Terraform） | 🧗 ramp |
| **[Azure](../../platforms/azure/)** | ✅ | ✅ ✅ ✅ | ✅ 3-lab CLI arc（`az`） | 🧗 + Entra/身份 ✋ |
...
**Agent Skills** —— 仓库自带四个 [`.claude/skills/`](../../.claude/skills/)，把方法论变成可调用的 AI 工作流：**platform-ramp**（诚实地上手任何平台）、**honesty-audit**（把声明分类 ✋/🧗/过度声明）、**author-module**（用仓库的声音写新章）、**runnable-lab**（把概念做成自验证 drill）。
