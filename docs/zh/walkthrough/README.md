...
存在，所以叙述者用故事自己的声音、在立足点切换的那一刻把界说出来。第一集里它切换一次
—— 从作者真的做过的分段与地址规划，切到
...
`<episode>.en.md` 和 `<episode>.zh.md` 并排放在那个目录里，**不在**
...
| | |
| --- | --- |
| **plate** —— `reference-office.plate.json` | 这层楼**是什么**：有哪些空间、谁挨着谁、怎么从一处走到另一处。各集共享，因为第二集是同一间办公室。 |
| **episode** —— `<episode>.floor.json` | 这一集**对它说了什么**：物体面板和拍级镜头。第一集里那些面板是几何的五倍大，而且每一个字都在讲网络。 |
...
1. **按「它讲什么」来定，不是按「它是第几步」。** 这里的顺序是它自己的：第一集横跨两条
...
后续的走读，在真的有什么值得说出口的时候才写，一次一集，像
[`toolbox/`](../../../toolbox/README.md) 那样生长。没有目标集数。
...
`--freeze <episode>` 会盖上 `published:` 和每个源的指纹。此后某个源被改动，会被报告成
