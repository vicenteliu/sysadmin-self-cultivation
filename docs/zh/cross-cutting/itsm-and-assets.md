...
3. **可答性** —— ✅ [`labs/permission-sprawl/`](labs/permission-sprawl)
   ...
   随叫随失败。
4. **人手** —— ✅ [`labs/help-desk-queue/`](labs/help-desk-queue)，规格没要过它，
   [`build-out/13`](../build-out/13-the-help-desk.md) 要过。*每五十个用户配一个*，换成一条能说出
   这个数为什么是这个数、以及那个比例从哪里开始不再对得上却说不出口的队列。它住在这里，是因为一个
   人手数字是一句关于 estate 的声称 —— 就是第一个 drill 对账的那个 estate —— 而它只有带着那个
   estate 才活得下来。
