...
它旁边是 [`counts.json`](../../../docs/counts.json) —— 散文对这个仓库自己陈述的每一个计数，
以及承载每一个计数的措辞。[`check.py`](../../../check.py) 从磁盘算出每一种数字，让说错的数字失败。
门面页只写那个文件里列出的种类，新种类先加进那个文件：没有种类锚住的数字，就是没有东西在检查的数字。
