...
summary: "对着一个沙箱账号做的三次 guided run —— 可拆、命令行优先。读一个子网和配一个子网是两项不同的技能；这些逼你做第二件。"
---
# AWS —— Guided run
...
对着一个沙箱账号做的三次 guided run —— 可拆、命令行优先。读一个子网和配一个子网是两项不同的技能；
这些逼你做第二件。
...
这里每一次 guided run 都是**命令行优先**的，而那是一个教学选择，不是偏好。控制台是用来*看*的；命令行是
