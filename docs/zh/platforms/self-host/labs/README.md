...
上面那条三节弧需要一台开了嵌套虚拟化的笔记本。还有一个 lab **什么都不需要** —— 一个纯本地、只用标
准库、能自我验证的演练，接着那篇[运维篇](../operations.md)：

### `raid-buys-time/` —— RAID 买给你的是时间 ✅ 已建（纯本地）

第三步底下的算术：一个镜像会把 `rm` 复制过去，重建窗口随盘容量变长，在机队规模上那个窗口里的第二次
故障是一年一次的事件、而它更大的那一半是换盘等待时间，今天的盘容量上一次 RAID 5 重建预期会在幸存者上
撞上读错误，而 RAID 6 把它压低一千倍。见 **[`raid-buys-time/`](raid-buys-time/)**。

```bash
python3 raid-buys-time/raid_drill.py   # exit 0 = 那些教训成立；在 CI 里跑
```
