...
## Lab —— RAID 买给你的是时间，而重建就是你花掉它的时候 ✅ 可跑

**给上面第一个要点垫上算术。** 一个纯本地、只用 stdlib 的 drill，用厂商自己的数字把那句话的每个分句
算出来：一个镜像会把 `rm` 复制过去，一块 20 TB 的盘是一次 37 小时的重建，一万个阵列在 2% 年故障率下、
盘还在采购路上时每年因窗口内的第二次故障丢掉 1.5 个、备件架备好时丢 0.5 个，20 TB 盘上的一次 RAID 5
重建预期会在幸存者上撞上读错误，而 RAID 6 把整件事压低一千倍 —— *备件是消耗品*，标上了价。

```bash
python3 platforms/self-host/labs/raid-buys-time/raid_drill.py
```

exit `0` 表示五条教训都成立（兼作 CI 检查）。见 [`labs/raid-buys-time/`](labs/raid-buys-time)。
