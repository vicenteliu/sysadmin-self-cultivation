...
| 走读 | [`walkthrough/`](walkthrough/README.md) —— 第二条路线，是念的不是读的：格式、五条决策（[ADR-0009](../../docs/adr/0009-the-walkthrough-ships-its-script-not-its-audio.md)–[0013](../../docs/adr/0013-godot-is-a-design-tool-and-the-floor-keeps-one-palette.md)）以及 viewer 的注册都已写完，而且**目前共两篇走读已建成** —— 01 · 网络（106 拍）与 02 · 第一个星期一（93 拍），各两种语言，播放在同一张可交互的二维楼面上。稿子进仓库，音频永远不进。像 `toolbox/` 那样一次长一篇，没有目标篇数 | ✅ |
