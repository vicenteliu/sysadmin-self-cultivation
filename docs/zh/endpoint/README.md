...
| | 它回答什么 |
|---|---|
| [`provisioning.md`](provisioning.md) | 一个平台在要同时覆盖 Windows、macOS **和** Linux 时决定什么 —— 两个发放时代以及你的估算面实际在哪一个里、三个操作系统真正共享的是什么，以及为什么硬件多样性是约束而不是渗漏 |
| [`management.md`](management.md) | 机器到了人手上之后 MDM 管什么 —— 你租来的那个管理面、为什么*已纳管*、*受管中*和*合规*是三个不同的断言、一套 Apple 估算面的形状告诉你什么，以及那个带时间轴的作用域问题 |
| [`encryption-and-keys.md`](encryption-and-keys.md) | 这一页称之为*真正的活*却从未展示过的那件事 —— 为什么托管是把风险搬走而不是移除、位于中心的那个循环依赖，以及那些活得比它们所解锁的机器更久的密钥 |
