...
| [`debug-ladder.md`](debug-ladder.md) | One command per rung of the network debug ladder, chosen for what it **eliminates** rather than what it reports — and why *refused* versus *timed out* carries more information than the rest of the page — **🔨** |
| [`vpn-and-remote-access.md`](vpn-and-remote-access.md) | What actually happens between clicking connect and reaching the thing — five decisions somebody owns, why the address you hand a remote user shows up in every log afterwards, and why split tunnelling breaks on DNS rather than on routing — **🔨** |
